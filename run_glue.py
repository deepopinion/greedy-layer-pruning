...
class ModelArguments:
    ...
    prune_method: Optional[str] = field(
        default="none", metadata={"help": "Pruning method to use: {none, cb-layers, top-layers, bottom-layers}"}
    )
    prune_n_layers: int = field(
        default=0, metadata={"help": "Number of layers to prune with the given method."}
    )
...
def main():
    ...
    if(model_args.prune_method == "prune-greedy"):
        ...
        layer_file_path = os.path.join(current_path, "layer_files/", f"{model.name_or_path}_{data_args.task_name}_greedy.txt")
        ...
        model.prune_layers([i for i in range(first_layer_to_prune, config.num_hidden_layers)])
